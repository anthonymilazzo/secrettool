...
function createSecretEnvs(secretsConfig: {
  envs: [string];
  project: string;
  secrets: [string];
}, secret: string): void {
...
function main(): void {
  try {
    ...
    const secretsConfig: { envs: [string]; project: string; secrets: [string] } =
      loadSecretsConfig(filename);

    displayMeta(secretsConfig);

    displayEnvs(secretsConfig);

    checkAllEnvs(secretsConfig);
